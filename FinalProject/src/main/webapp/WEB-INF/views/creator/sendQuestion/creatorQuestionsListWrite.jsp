...
<script>
	$(()=>{
		...
		$("#hQnAWCan6").click(()=>{//취소버큰 클릭 -> 목록으로돌아가기
			$("#hQnAWriteD33ulFrm").attr("action", "/another/creator/CreatorQuestion/list" );
			$("#hQnAWriteD33ulFrm").submit();			
		});
		
		$("#hQnAWSubmit9").click(()=>{//등록		
			$("#hQnAWriteD33ulFrm").attr("action", "/another/creator/CreatorQuestion/writeOk" );
			$("#hQnAWriteD33ulFrm").submit();			
		});
	});
	...
	<div id="hQnAWriteD33">
		...
		<form method="post" id="hQnAWriteD33ulFrm">
			<ul id="hQnAWriteD33ul">
				<li>
					<div>회원정보</div>
					<div>${creatorId}</div>	<!-- 세션에저장되어잉ㅆ는아이디 -->
				</li>
				<li>
					<div>제목</div>
